...
export const AuditoriumCanvas: React.FC = () => {
	...
	const streamBoundsRef = useRef<HTMLDivElement>(null);
	...
	useEffect(() => {
		...
		const scene = new AuditoriumScene(containerRef.current, initialSeats, {
			onScreenBoundsChange: (bounds) => {
				if (!streamBoundsRef.current) return;

				streamBoundsRef.current.style.left = `${bounds.left}px`;
				streamBoundsRef.current.style.top = `${bounds.top}px`;
				streamBoundsRef.current.style.width = `${bounds.width}px`;
				streamBoundsRef.current.style.height = `${bounds.height}px`;
			},
			...
			{sceneState !== "LOBBY" && (
				...
				>
					<iframe
						...
						src="https://www.youtube.com/embed/KIDah3KNmco?autoplay=1&mute=1&playsinline=1&enablejsapi=1"
						frameBorder="0"
          ...
          "
            ...
            ------------------------------------------------- */}
						<p className="text-slate-400 text-sm sm:text-base leading-relaxed">
							Please rotate your phone to experience the
							auditorium.
						</p>

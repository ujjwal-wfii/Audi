...
export class AuditoriumScene {
  ...
  ------------------------------------------------------------- */
	...
	// -------------------------------------------------------------
	// START YOUTUBE VIDEO
	// Creates a fresh iframe so autoplay starts again.
	// -------------------------------------------------------------
	private startYouTubeVideo() {
		if (!this.youtubeScreen) return;

		const wrapper = this.youtubeScreen.element;

		// Don't create another iframe if one already exists.
  ...
  ------------------------------------------------------------- */
	private buildAuditoriumHall() {
		...
		// this.createYouTubeScreen(); lucky for now, we don't want to load the youtube video until the user is seated
  ...
  ------------------------------------------------------------- */
	private buildSeatingArea(seats: SeatData[]) {
		...
		seats.forEach((seat) => {
			...
			frontBolster.position.set(0, 0.375, -0.235);
			...
			backrest.position.set(0, 0.995, 0.225);
			...
			const createArmrest = (xPos: number) => {
				...
				topCap.position.set(0, 0.825, 0.02);

				armGroup.add(topCap);

				// Thin brass trim
				const brassTrim = new THREE.Mesh(
					new THREE.BoxGeometry(0.012, 0.028, 0.48),
					brassMat,
				);

				brassTrim.position.set(0.035, 0.825, 0.02);
			...
			if (seat.number === 1 || seat.number === 7) {
				...
				endPanel.position.set(isLeftAisle ? -0.36 : 0.36, 0.52, -0.04);

				seatGroup.add(endPanel);

				// Small blue aisle indicator
				const led = new THREE.Mesh(
					new THREE.BoxGeometry(0.012, 0.035, 0.055),
					ledMat,
				);
	...
	private animate() {
		...
		if (this.youtubeScreen) {
			if (this.currentState === "LOBBY") {
				...
				this.youtubeScreen.visible = this.isScreenVisibleFromCamera();
			}
		}
